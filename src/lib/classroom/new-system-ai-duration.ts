...
2. 上游知识要求已通过 sourceKnowledgePointIds 映射到教材化课程节点，并已按 groupId/groupName 形成可共同讲解的 teachingClusters；原始节点名称和解释不要求逐字保留。时间分配的最小单位是知识簇，不是单个知识点。${fixed ? "总 durationMin 已锁定，只根据各知识簇的共同解释主线、抽象度、依赖深度与学生基础分配时间。" : "在上述范围内选择总 durationMin，用更多时间深化已确认结构，不要在这个阶段删除或扩张知识点。"}确定总时长后再分配知识簇预算，最后才生成课程；不要根据页数反推或扩大总时长。
