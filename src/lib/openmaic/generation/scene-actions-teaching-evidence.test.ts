...
describe("official-baseline narration evidence boundary", () => {
  it("keeps CoTeach evidence out of the upstream action prompt", async () => {
    ...
    expect(prompt).not.toContain("必须根据独立测试判断");
    expect(prompt).not.toContain("不能复用训练集");
    expect(prompt).not.toContain("检测重点：说明判断依据");
