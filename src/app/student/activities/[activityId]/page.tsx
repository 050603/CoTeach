...
type Activity = { id: string; type: string; title: string; description: string | null; isOpen: boolean; offering: { id: string; name: string; status: string }; chapter: { title: string }; config?: { content?: string; url?: string; questions?: Array<{ id: string; title: string; required?: boolean }> }; progress: { status: string; progressData?: { answer?: string; answers?: Record<string, string> } }; instance: ActivityInstance | null; instances?: ActivityInstance[] };
...
export default function StudentActivityPage() {
  ...
  if (error && !activity) return <StudentShell><section className="pbl-content-card mx-auto max-w-2xl p-8"><h1 className="text-xl font-semibold">暂时无法打开活动</h1><p role="alert" className="mt-3 text-sm text-[var(--pbl-danger)]">{error}</p><button type="button" className="mt-6 min-h-11 rounded-xl bg-[var(--pbl-student)] px-6 text-sm font-semibold text-white" onClick={() => window.location.reload()}>重新加载</button><Link href="/student" className="ml-5 text-sm text-[var(--pbl-text-muted)]">返回学习空间</Link></section></StudentShell>;
  if (!activity) return <StudentShell><section aria-busy="true" aria-label="正在加载活动" className="pbl-content-card mx-auto max-w-3xl space-y-5 p-8"><div className="h-6 w-24 animate-pulse rounded-lg bg-[var(--pbl-bg)] motion-reduce:animate-none" /><div className="h-10 w-2/3 animate-pulse rounded-lg bg-[var(--pbl-bg)] motion-reduce:animate-none" /><div className="h-40 animate-pulse rounded-xl bg-[var(--pbl-bg)] motion-reduce:animate-none" /><p role="status" className="text-sm text-[var(--pbl-text-muted)]">正在加载学习内容…</p></section></StudentShell>;
