...
const schema = z.object({ name: z.string().trim().min(1).max(160).optional(), description: z.string().max(20_000).optional(), term: z.string().max(80).optional(), status: z.enum(["draft", "open", "finished", "archived"]).optional(), startsAt: z.string().datetime().nullable().optional(), endsAt: z.string().datetime().nullable().optional(), coverImageUrl: z.string().url().nullable().optional(), version: z.number().int().positive().optional() });
