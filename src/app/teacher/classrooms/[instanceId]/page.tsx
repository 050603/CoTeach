...
export default function ClassroomMonitor() {
  ...
  return <TeacherPlatformPage>
    ...
    <div className="pbl-workspace-content">
      <header className="pbl-page-heading"><div><p className="text-xs font-semibold tracking-widest text-[var(--pbl-teacher)]">课堂学习记录</p><h1 className="mt-3 font-serif text-3xl font-semibold md:text-4xl">{data?.instance.title ?? "课堂学习记录"}</h1>{data && <span className="mt-5 inline-flex rounded-full bg-[var(--pbl-teacher-soft)] px-4 py-2 text-sm text-[var(--pbl-teacher)]">{status === "teaching" ? "授课中" : status === "finished" ? "已结束 · 记录留存" : "待授课"}</span>}</div>{data?.instance.coverImageUrl ? <div className="relative aspect-video w-[360px] max-w-[38%] shrink-0 overflow-hidden rounded-[14px] border border-white/70 shadow-lg"><ResilientImage src={data.instance.coverImageUrl} alt={`${data.instance.title}课堂封面`} fill unoptimized className="object-cover" /></div> : null}</header>
      ...
      {data ? <TeacherExperimentResults instanceId={instanceId} offeringId={data.instance.offeringId} /> : null}
    </div>
  </TeacherPlatformPage>;
}
