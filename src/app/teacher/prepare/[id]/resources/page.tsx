...
export default function TeacherResourcesPage() {
  ...
  if (!hydrated) {
    return (
      <DashboardShell backHref={`/teacher/prepare/${encodeURIComponent(params.id)}/preview`} backLabel="返回发布中心" role="teacher" userName={user.name} variant="bare">
  ...
  if (!teacherClassroomId) {
    return (
      <DashboardShell
        backHref={`/teacher/prepare/${encodeURIComponent(course.id)}/preview`}
        backLabel="返回发布中心"
        role="teacher"
        userName={user.name}
        variant="bare"
