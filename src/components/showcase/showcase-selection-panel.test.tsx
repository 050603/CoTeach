...
describe("teacher presenter selection", () => {
  ...
  it("shows and preserves the presenter count from the resource package", () => {
    const save = vi.fn(async () => undefined);
    const data = {
      courseId: "course", queueConfig: { schemaVersion: 2, selectionMode: "teacher-selected", selectedStudentIds: [], presenterCount: 5, presentationSec: 90, discussionSec: 30, transitionSec: 5 },
      students: [{ studentId: "s1", name: "甲", artifacts: [] }], queue: [],
    } as unknown as ShowcaseData;
    render(<ShowcaseSelectionPanel data={data} busy={false} save={save} />);
    expect(screen.getByText(/资源包安排 5 人/)).toBeTruthy();
    fireEvent.click(screen.getByLabelText("选择甲现场汇报"));
    fireEvent.click(screen.getByRole("button", { name: "保存汇报名单与时间" }));
    expect(save).toHaveBeenCalledWith(expect.objectContaining({ presenterCount: 5, presentationSec: 90, discussionSec: 30, transitionSec: 5 }));
  });
});
