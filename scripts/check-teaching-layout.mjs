...
const output = fs.mkdtempSync(path.join(os.tmpdir(), 'openpbl-teaching-'));
...
const scene = {
  ...
  content: { type: 'slide', canvas: { id: 'layout-slide', viewportSize: 1000, viewportRatio: 0.5625, theme: { backgroundColor: '#ffffff', themeColors: ['#344A6A'], fontColor: '#1F2933', fontName: 'Noto Sans SC' }, background: { type: 'solid', color: '#ffffff' }, elements: [
    ...
    { id: 'image', type: 'image', src: '/brand/logo-horizontal.png', left: 48, top: 385, width: 240, height: 70, rotate: 0, fixedRatio: true },
  ] } },
};
const fixtureCourse = {
...
const profiles = [
  ['desktop', 1440, 900, false], ['pad-portrait', 768, 1024, true], ['pad-landscape', 1024, 768, true],
...
const scenarios = [
  ...
  ['quick-preparation', '/teacher/prepare/layout-teaching/verify', '[aria-label="描述课程生成要求"]', async () => {}],
  ['preparation', '/teacher/prepare/layout-teaching/verify/edit', '[aria-label="课程备课流程"]', async page => {
    const picker = page.getByRole('combobox', { name: '切换备课步骤' });
    if (await picker.isVisible()) { await picker.selectOption('evaluationPlan'); await picker.selectOption('base'); }
  }],
  ...
  ['player', '/teacher/prepare/layout-teaching/preview', '[role="tablist"]', async page => {
    await page.getByRole('tab', { name: '学生 AI 课堂实景' }).click();
    await page.locator('[data-testid="scene-list"]').waitFor();
...
try {
  for (const [device, width, height, touch] of select(profiles, process.env.TEACHING_DEVICES)) {
    for (const [id, route, ready, exercise] of select(scenarios, process.env.TEACHING_SCENARIOS)) {
      const context = await browser.newContext({ viewport: { width, height }, isMobile: touch && width < 1024, hasTouch: touch });
      ...
      if (id === 'student-player') course.currentStageIndex = 1;
      ...
      const report = { id, device, width, height, sourceCss: process.argv.includes('--source-css'), errors, missingFixtures, failedResources, interceptedWrites };
      try {
        ...
        await page.evaluate(async () => { await document.fonts.ready; await Promise.all([...document.images].map(img => Promise.race([img.decode().catch(() => {}), new Promise(resolve => setTimeout(resolve, 5000))]))); });
...
console.log(JSON.stringify({ output, checks: reports.length, failures: reports.filter(r => r.failed).length }));
