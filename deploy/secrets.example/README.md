...
- `s3_access_key.txt` and `s3_secret_key.txt`: restricted credentials for the
  manual essential-backup bucket only.
- `restic_password.txt`: a separate high-entropy password for encrypted manual
  student-account and first-questionnaire snapshots. Keep an offline copy; a
  lost Restic password cannot be recovered from the bucket.
